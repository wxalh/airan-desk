...
void MouseCursorMonitorWin::Capture() {
  ...
  if (!GetCursorInfo(&cursor_info)) {
    LOG_ERROR("Unable to get cursor info. Error = {}", GetLastError());
    return;
  }
...
DesktopRect MouseCursorMonitorWin::GetScreenRect() {
  ...
  if (screen_ < 0 ||
      screen_ > static_cast<ScreenId>((std::numeric_limits<DWORD>::max)())) {
    return DesktopRect();
  }
  BOOL result =
      EnumDisplayDevices(NULL, static_cast<DWORD>(screen_), &device, 0);
  if (!result)
    return DesktopRect();
