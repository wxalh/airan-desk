...
constexpr int kAiranMainCapturerFailureThreshold = 3;
constexpr std::chrono::milliseconds kAiranMainCapturerHoldTime(3000);
...
void FallbackDesktopCapturerWrapper::SetAiranCaptureCallback(
    airan::media::AiranCaptureCallback* callback) {
  main_capturer_->SetAiranCaptureCallback(callback);
  secondary_capturer_->SetAiranCaptureCallback(callback);
}

void FallbackDesktopCapturerWrapper::CaptureFrame() {
  RTC_DCHECK(callback_);
  if (!mainCapturerAvailableForProbe()) {
...
bool FallbackDesktopCapturerWrapper::SelectSource(SourceId id) {
  ...
  if (!main_capturer_result)
    openMainCapturerBreaker(Result::ERROR_PERMANENT);
...
void FallbackDesktopCapturerWrapper::OnCaptureResult(
  ...
  if (result == Result::SUCCESS) {
    main_capturer_failure_count_ = 0;
    main_capturer_breaker_open_ = false;
    main_capturer_permanent_error_ = false;
    callback_->OnCaptureResult(result, std::move(frame));
    return;
  }

  openMainCapturerBreaker(result);
  secondary_capturer_->CaptureFrame();
}

bool FallbackDesktopCapturerWrapper::mainCapturerAvailableForProbe() const {
  if (main_capturer_permanent_error_)
    return false;
  if (!main_capturer_breaker_open_)
    return true;
  return std::chrono::steady_clock::now() - main_capturer_breaker_opened_at_ >=
         kAiranMainCapturerHoldTime;
}

void FallbackDesktopCapturerWrapper::openMainCapturerBreaker(Result result) {
  ++main_capturer_failure_count_;
  if (result == Result::ERROR_PERMANENT) {
    main_capturer_permanent_error_ = true;
    main_capturer_breaker_open_ = true;
  } else if (main_capturer_failure_count_ >=
             kAiranMainCapturerFailureThreshold) {
    main_capturer_breaker_open_ = true;
  }

  if (main_capturer_breaker_open_) {
    main_capturer_breaker_opened_at_ = std::chrono::steady_clock::now();
    LOG_WARN("Airan desktop capture primary breaker opened; failures={}, permanent={}",
             main_capturer_failure_count_,
             (main_capturer_permanent_error_ ? 1 : 0));
  }
}
