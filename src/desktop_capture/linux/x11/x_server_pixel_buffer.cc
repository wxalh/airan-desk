...
#include "desktop_capture/linux/x11/x_error_trap.h"
#include "desktop_capture/linux/x11/x_window_property.h"
...
void XServerPixelBuffer::InitShm(const XWindowAttributes& attributes) {
    ...
    if (shm_segment_info_->shmid != -1) {
      ...
      if (shmat_result != reinterpret_cast<void*>(-1)) {
        ...
        if (error_trap.GetLastErrorAndDisable() != 0)
          xshm_attached_ = false;
        if (xshm_attached_) {
          LOG_VERBOSE("Using X shared memory segment {}", shm_segment_info_->shmid);
        }
      }
    } else {
  ...
  LOG_VERBOSE("Using X shared memory extension v{}.{} with{} pixmaps.", major, minor, (have_pixmaps ? "" : "out"));
}
