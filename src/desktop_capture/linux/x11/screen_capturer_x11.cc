...
#include "desktop_capture/linux/x11/x_server_pixel_buffer.h"
#include "desktop_capture/screen_capture_frame_queue.h"
#include "desktop_capture/screen_capturer_helper.h"
...
bool ScreenCapturerX11::Init(const DesktopCaptureOptions& options) {
  ...
  if (root_window_ == BadValue) {
    LOG_ERROR("Unable to get the root window");
    DeinitXlib();
    return false;
  }
  ...
  if (gc_ == nullptr) {
    LOG_ERROR("Unable to get graphics context");
    DeinitXlib();
    return false;
  }
  ...
  if (XFixesQueryExtension(display(), &xfixes_event_base_,
    ...
    LOG_INFO("X server does not support XFixes.");
  }
  ...
  if (!x_server_pixel_buffer_.Init(atom_cache_.get(),
    ...
    LOG_ERROR("Failed to initialize pixel buffer.");
    return false;
  }
  ...
  if (!SelectSource(kFullDesktopScreenId)) {
    LOG_ERROR("Airan X11 screen capturer requires XRandR monitor selection.");
    return false;
  }
...
void ScreenCapturerX11::InitXDamage() {
  ...
  if (!XDamageQueryExtension(display(), &damage_event_base_,
                             &damage_error_base_)) {
    LOG_INFO("X server does not support XDamage.");
    return;
  }
  ...
  if (!damage_handle_) {
    LOG_ERROR("Unable to initialize XDamage.");
    return;
  }
  ...
  if (!damage_region_) {
    ...
    LOG_ERROR("Unable to create XFixes region.");
    return;
  }
  ...
  use_damage_ = true;
  LOG_INFO("Using XDamage extension.");
}
  ...
  if (XRRQueryExtension(display(), &randr_event_base_, &error_base_ignored) &&
    ...
    if (major_version > 1 || (major_version == 1 && minor_version >= 5)) {
      ...
      if (get_monitors_ && free_monitors_) {
        use_randr_ = true;
        LOG_INFO("Using XRandR extension v{}.{}.", major_version, minor_version);
        monitors_ =
        ...
        LOG_ERROR("Unable to link XRandR monitor functions.");
      }
    } else {
      LOG_ERROR("XRandR entension is older than v1.5.");
    }
  } else {
    LOG_ERROR("X server does not support XRandR.");
  }
}
  ...
  if (selected_monitor_name_) {
    ...
    for (int i = 0; i < num_monitors_; ++i) {
      ...
      if (selected_monitor_name_ == m.name) {
        LOG_INFO("XRandR monitor {} rect updated.", m.name);
        ...
        if (!pixel_buffer_rect.ContainsRect(selected_monitor_rect_)) {
          ...
          LOG_WARN("Cropping selected monitor rect to fit the pixel-buffer.");
    ...
    LOG_INFO("XRandR selected monitor {} lost.", selected_monitor_name_);
...
void ScreenCapturerX11::CaptureFrame() {
  ...
  if (!x_server_pixel_buffer_.is_initialized()) {
    ...
    LOG_ERROR("Pixel buffer is not initialized.");
  ...
  if (!result) {
    LOG_WARN("Temporarily failed to capture screen.");
...
bool ScreenCapturerX11::SelectSource(SourceId id) {
  ...
  if (id == kFullDesktopScreenId) {
    ...
    if (selected_index < 0 && num_monitors_ > 0) {
      selected_index = 0;
    }
    if (selected_index < 0) {
      return false;
    }
    id = static_cast<SourceId>(monitors_[selected_index].name);
    LOG_INFO("Airan X11 screen capturer maps full desktop source "
             "to monitor atom {}", id);
  }

  for (int i = 0; i < num_monitors_; ++i) {
    if (id == static_cast<SourceId>(monitors_[i].name)) {
      LOG_INFO("XRandR selected source: {}", id);
      XRRMonitorInfo& m = monitors_[i];
...
bool ScreenCapturerX11::HandleXEvent(const XEvent& event) {
  if (use_damage_ && (event.type == damage_event_base_ + XDamageNotify)) {
    ...
    UpdateMonitors();
    LOG_INFO("XRandR screen change event received.");
    return false;
...
void ScreenCapturerX11::ScreenConfigurationChanged() {
  ...
  if (!x_server_pixel_buffer_.Init(atom_cache_.get(),
                                   DefaultRootWindow(display()))) {
    LOG_ERROR("Failed to initialize pixel buffer after screen "
...
std::unique_ptr<DesktopCapturer> ScreenCapturerX11::CreateRawScreenCapturer(
  ...
  LOG_INFO("ScreenCapturerX11::CreateRawScreenCapturer creates "
  ...
  if (!capturer->Init(options)) {
    LOG_INFO("ScreenCapturerX11::CreateRawScreenCapturer "
