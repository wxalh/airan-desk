...
#include "desktop_capture/linux/x11/window_capturer_x11.h"
...
#include "desktop_capture/linux/x11/window_finder_x11.h"
#include "desktop_capture/linux/x11/window_list_utils.h"
...
WindowCapturerX11::WindowCapturerX11(const DesktopCaptureOptions& options)
  ...
  if (XCompositeQueryExtension(display(), &event_base, &error_base) &&
    ...
    LOG_INFO("Xcomposite extension not available or too old.");
  }
...
bool WindowCapturerX11::FocusOnSelectedSource() {
  ...
  if (status == 0) {
    LOG_ERROR("Failed to query for the root window.");
    return false;
  }
...
void WindowCapturerX11::CaptureFrame() {
  ...
  if (!x_server_pixel_buffer_.IsWindowValid()) {
    LOG_ERROR("The window is no longer valid.");
  ...
  if (!has_composite_extension_) {
    ...
    LOG_ERROR("No Xcomposite extension detected.");
    callback_->OnCaptureResult(Result::ERROR_PERMANENT, nullptr);
    return;
  }
  ...
  if (!x_server_pixel_buffer_.CaptureRect(DesktopRect::MakeSize(frame->size()),
                                          frame.get())) {
    LOG_WARN("Temporarily failed to capture winodw.");
    ...
    if (xce.window == selected_window_) {
      if (!DesktopRectFromXAttributes(xce).equals(
        ...
        if (!x_server_pixel_buffer_.Init(&atom_cache_, selected_window_)) {
          LOG_ERROR("Failed to initialize pixel buffer after resizing.");
        }
      }
    }
  }
...
std::unique_ptr<DesktopCapturer> WindowCapturerX11::CreateRawWindowCapturer(
  ...
  LOG_INFO("WindowCapturerX11::CreateRawWindowCapturer creates "
