...
#include "desktop_capture/linux/wayland/shared_screencast_stream.h"
...
// format-types.h was removed in PipeWire >= 0.3.58; its content is now in format.h
#if __has_include(<spa/param/format-types.h>)
...
#include "desktop_capture/linux/wayland/egl_dmabuf.h"
#include "desktop_capture/linux/wayland/screencast_stream_utils.h"
...
class SharedScreenCastStreamPrivate {
 ...
 private:
  ...
  bool ProcessDMABuffer(pw_buffer* buffer,
                        DesktopFrame& frame,
                        const DesktopVector& offset);
  void EmitAiranCpuFrameDescriptor(const DesktopFrame& frame,
...
void SharedScreenCastStreamPrivate::OnCoreError(void* data,
  ...
  LOG_ERROR("PipeWire remote error: {}", message);
...
void SharedScreenCastStreamPrivate::OnCoreInfo(void* data,
  ...
  LOG_INFO("PipeWire server version: {}", stream->pw_server_version_.ToStringView());
}
...
void SharedScreenCastStreamPrivate::OnStreamStateChanged(
  ...
  LOG_INFO("PipeWire stream state: {}", pw_stream_state_as_string(state));

  switch (state) {
    case PW_STREAM_STATE_ERROR:
      LOG_ERROR("PipeWire stream state error: {}", error_message);
      break;
...
void SharedScreenCastStreamPrivate::OnStreamParamChanged(
  ...
  if (true) {
    LOG_INFO("PipeWire stream format changed:\n"
             "    Format: {} ({})\n"
             "{}    Size: {} x {}\n"
             "    Framerate: {}/{}",
             static_cast<int>(that->spa_video_format_.format),
             spa_debug_type_find_name(spa_type_video_format,
                                     that->spa_video_format_.format),
             has_modifier ? fmt::format("    Modifier: {}\n", that->modifier_) : "",
             that->spa_video_format_.size.width,
             that->spa_video_format_.size.height,
...
void SharedScreenCastStreamPrivate::OnStreamProcess(void* data) {
  ...
  if (header && (header->flags & SPA_META_HEADER_FLAG_CORRUPTED)) {
    LOG_INFO("Dropping corrupted buffer");
    if (that->observer_) {
...
bool SharedScreenCastStreamPrivate::StartScreenCastStream(
  ...
  if (!InitializePipeWire()) {
    LOG_ERROR("Unable to open PipeWire library");
    return false;
  }
  ...
  if (pw_thread_loop_start(pw_main_loop_) < 0) {
    LOG_ERROR("Failed to start main PipeWire loop");
    return false;
  }

  pw_client_version_ = PipeWireVersion::Parse(pw_get_library_version());
  LOG_INFO("PipeWire client version: {}", pw_client_version_.ToStringView());
...

  {
    ...
    if (!pw_core_) {
      LOG_ERROR("Failed to connect PipeWire context");
      return false;
    }
    ...
    if (pw_stream_connect(pw_stream_, PW_DIRECTION_INPUT, pw_stream_node_id_,
                          ...
                          params.size()) != 0) {
      LOG_ERROR("Could not connect receiving stream");
      return false;
    }
  }
  return true;
...
void SharedScreenCastStreamPrivate::UpdateScreenCastStreamResolution(
    ...
    uint32_t height) {
  if (!width || !height) {
    LOG_WARN("Bad resolution specified: {}x{}", width, height);
    return;
  }
  if (!pw_main_loop_) {
    LOG_WARN("No main pipewire loop, ignoring resolution change");
    return;
  }
  if (!renegotiate_) {
    LOG_WARN("Can not renegotiate stream params, ignoring resolution change");
    return;
  }
...
void SharedScreenCastStreamPrivate::UpdateScreenCastStreamFrameRate(
    uint32_t frame_rate) {
  if (!pw_main_loop_) {
    LOG_WARN("No main pipewire loop, ignoring frame rate change");
    return;
  }
  if (!renegotiate_) {
    LOG_WARN("Can not renegotiate stream params, ignoring frame rate change");
    return;
  }
...
void SharedScreenCastStreamPrivate::ProcessBuffer(pw_buffer* buffer) {
  ...
  if (spa_buffer->datas[0].chunk->flags & SPA_CHUNK_FLAG_CORRUPTED) {
    LOG_INFO("Dropping buffer with corrupted or missing data");
    if (observer_) {
  ...
  if (spa_buffer->datas[0].type == SPA_DATA_MemFd &&
    ...
    LOG_INFO("Dropping buffer with empty data");
    if (observer_) {
  ...
  if (videocrop_metadata &&
    ...
    LOG_ERROR("Stream metadata sizes are wrong!");
  ...
  std::unique_ptr<SharedDesktopFrame> airan_frame;
  bool has_callback = false;
  {
    ...
    if (spa_buffer->datas[0].type == SPA_DATA_MemFd) {
      ...
      bufferProcessed =
          ProcessDMABuffer(buffer, *queue_.current_frame(), offset);
    }
...

    {
      ...
      airan_frame = latest_available_frame_->Share();
  ...
  if (airan_frame) {
    EmitAiranCpuFrameDescriptor(*airan_frame, TimeMicros());
  }
...
bool SharedScreenCastStreamPrivate::ProcessMemFDBuffer(
  ...
  if (!map) {
    LOG_ERROR("Failed to mmap the memory: {}", std::strerror(errno));
    return false;
  }
...
bool SharedScreenCastStreamPrivate::ProcessDMABuffer(
  ...
  if (!imported) {
    LOG_ERROR("Dropping DMA-BUF modifier: {} and trying to renegotiate stream parameters", modifier_);

    if (pw_server_version_ >= kDropSingleModifierMinVersion) {
      modifiers_.erase(std::remove(modifiers_.begin(), modifiers_.end(), modifier_),
                       modifiers_.end());
    } else {
      modifiers_.clear();
    }
...
void SharedScreenCastStreamPrivate::EmitAiranCpuFrameDescriptor(
    const DesktopFrame& frame,
    int64_t timestamp_us) {
  airan::media::AiranCaptureCallback* callback = nullptr;
  {
    MutexLock latest_frame_lock(&latest_frame_lock_);
    callback = airan_capture_callback_;
  }
  if (!callback || !frame.data() || frame.size().width() <= 0 ||
  ...
  auto owned_pixels = std::make_shared<std::vector<uint8_t>>(
      static_cast<size_t>(stride) * static_cast<size_t>(height));
  std::memcpy(owned_pixels->data(), frame.data(), owned_pixels->size());
  ...
  descriptor.device_id.platform = "linux";
  descriptor.device_id.id = "pipewire-cpu";
  descriptor.sync.type = airan::media::SyncType::kNone;
  descriptor.sync.waiter = airan::media::SyncWaiter::kNone;
  descriptor.visible_rect = {0, 0, width, height};
  descriptor.coded_size = {width, height};
  descriptor.planes.push_back(
      {owned_pixels->data(), stride, 0, width, height, 0});
  ...
  descriptor.current_encode_path = airan::media::EncodePath::CpuSoftwareEncode;
  descriptor.fallback_reason = airan::media::FallbackReason::CapabilityMissing;
  descriptor.release = airan::media::makeFrameRelease([owned_pixels]() {});
...
void SharedScreenCastStream::SetAiranCaptureCallback(
    airan::media::AiranCaptureCallback* callback) {
  private_->SetAiranCaptureCallback(callback);
}
