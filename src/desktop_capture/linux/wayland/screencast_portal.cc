...
#include "desktop_capture/linux/wayland/screencast_portal.h"
...
#include "desktop_capture/linux/wayland/scoped_glib.h"
#include "desktop_capture/linux/wayland/xdg_desktop_portal_utils.h"
...
void ScreenCastPortal::OnSessionClosedSignal(GDBusConnection* connection,
  ...
  LOG_INFO("Received closed signal from session.");
...
void ScreenCastPortal::SourcesRequest() {
  ...
  LOG_INFO("Requesting sources from the screen cast session.");
  g_dbus_proxy_call(
...
void ScreenCastPortal::OnSourcesRequested(GDBusProxy* proxy,
  ...
  if (!variant) {
    if (g_error_matches(error.get(), G_IO_ERROR, G_IO_ERROR_CANCELLED))
      return;
    LOG_ERROR("Failed to request the sources: {}", error->message);
    that->OnPortalDone(RequestResponse::kError);
    return;
  }

  LOG_INFO("Sources requested from the screen cast session.");

  Scoped<char> handle;
  g_variant_get_child(variant.get(), 0, "o", handle.receive());
  if (!handle) {
    LOG_ERROR("Failed to initialize the screen cast session.");
  ...
  LOG_INFO("Subscribed to sources signal.");
}
...
void ScreenCastPortal::OnSourcesRequestResponseSignal(
  ...
  LOG_INFO("Received sources signal from session.");

  uint32_t portal_response;
  g_variant_get(parameters, "(u@a{sv})", &portal_response, nullptr);
  if (portal_response) {
    LOG_ERROR("Failed to select sources for the screen cast session.");
    that->OnPortalDone(RequestResponse::kError);
    return;
  }
...
void ScreenCastPortal::OnStartRequestResponseSignal(GDBusConnection* connection,
  ...
  if (portal_response || !response_data) {
    LOG_ERROR("Failed to start the screen cast session.");
...
void ScreenCastPortal::OpenPipeWireRemote() {
  ...
  LOG_INFO("Opening the PipeWire remote.");
...
void ScreenCastPortal::OnOpenPipeWireRemoteRequested(GDBusProxy* proxy,
  ...
  if (!variant) {
    if (g_error_matches(error.get(), G_IO_ERROR, G_IO_ERROR_CANCELLED))
      return;
    LOG_ERROR("Failed to open the PipeWire remote: {}", error->message);
    that->OnPortalDone(RequestResponse::kError);
    return;
  }
  ...
  if (that->pw_fd_ == kInvalidPipeWireFd) {
    LOG_ERROR("Failed to get file descriptor from the list: {}", error->message);
    that->OnPortalDone(RequestResponse::kError);
    return;
  }
