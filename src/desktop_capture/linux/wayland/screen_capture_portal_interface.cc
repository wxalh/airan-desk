 ...
 */
#include "desktop_capture/linux/wayland/screen_capture_portal_interface.h"
...
void ScreenCapturePortalInterface::RequestSessionUsingProxy(
  ...
  if (!proxy) {
    ...
    if (g_error_matches(error.get(), G_IO_ERROR, G_IO_ERROR_CANCELLED))
      return;
    LOG_ERROR("Failed to get a proxy for the portal: {}", error->message);
    OnPortalDone(RequestResponse::kError);
    return;
  }

  LOG_INFO("Successfully created proxy for the portal.");
...
void ScreenCapturePortalInterface::OnSessionRequestResult(
  ...
  if (!variant) {
    ...
    if (g_error_matches(error.get(), G_IO_ERROR, G_IO_ERROR_CANCELLED))
      return;
    LOG_ERROR("Failed to request session: {}", error->message);
    OnPortalDone(RequestResponse::kError);
    return;
  }

  LOG_INFO("Initializing the session.");

  Scoped<char> handle;
  g_variant_get_child(variant.get(), /*index=*/0, /*format_string=*/"o",
                      &handle);
  if (!handle) {
    LOG_ERROR("Failed to initialize the session.");
    OnPortalDone(RequestResponse::kError);
    return;
  }
}
...
void ScreenCapturePortalInterface::RegisterSessionClosedSignalHandler(
  ...
  if (RequestResponseFromPortalResponse(portal_response) !=
    ...
    LOG_ERROR("Failed to request the session subscription.");
    OnPortalDone(RequestResponse::kError);
    return;
  }
  ...
  if (session_handle.empty()) {
    LOG_ERROR("Could not get session handle despite valid response");
    OnPortalDone(RequestResponse::kError);
    return;
  }
...
void ScreenCapturePortalInterface::OnStartRequestResult(GDBusProxy* proxy,
  ...
  if (!variant) {
    if (g_error_matches(error.get(), G_IO_ERROR, G_IO_ERROR_CANCELLED))
      return;
    LOG_ERROR("Failed to start the portal session: {}", error->message);
    OnPortalDone(RequestResponse::kError);
    return;
  }
  ...
  if (!handle) {
    LOG_ERROR("Failed to initialize the start portal session.");
    OnPortalDone(RequestResponse::kError);
    return;
  }

  LOG_INFO("Subscribed to the start signal.");
}
