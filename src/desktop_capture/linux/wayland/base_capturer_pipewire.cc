...
#include "desktop_capture/linux/wayland/restore_token_manager.h"
...
void BaseCapturerPipeWire::OnScreenCastRequestResult(RequestResponse result,
  ...
  if (result != RequestResponse::kSuccess ||
    ...
    LOG_ERROR("ScreenCastPortal failed: {}", static_cast<uint>(result));
...
void BaseCapturerPipeWire::SetAiranCaptureCallback(
    airan::media::AiranCaptureCallback* callback) {
  if (options_.screencast_stream()) {
    options_.screencast_stream()->SetAiranCaptureCallback(callback);
  }
}
