...
EglDmaBuf::EglDmaBuf() {
  if (!LoadEGL()) {
    LOG_ERROR("Unable to load EGL entry functions.");
    CloseLibrary(g_lib_egl);
    return;
  }

  if (!LoadGL()) {
    LOG_ERROR("Failed to load OpenGL entry functions.");
  ...
  if (!has_platform_base_ext || !has_platform_gbm_ext ||
    ...
    LOG_ERROR("One of required EGL extensions is missing");
    return;
  }
  ...
  if (egl_.display == EGL_NO_DISPLAY) {
    LOG_ERROR("Failed to obtain default EGL display: {}\nDefaulting to using first available render node", FormatEGLError(EglGetError()));
    ...
    if (drm_fd_ < 0) {
      LOG_ERROR("Failed to open drm render node: {}", strerror(errno));
      return;
    }
    ...
    if (!gbm_device_) {
      LOG_ERROR("Cannot create GBM device: {}", strerror(errno));
      close(drm_fd_);
      return;
    }
  ...
  if (egl_.display == EGL_NO_DISPLAY) {
    LOG_ERROR("Error during obtaining EGL display: {}", FormatEGLError(EglGetError()));
    return;
  }

  EGLint major, minor;
  if (EglInitialize(egl_.display, &major, &minor) == EGL_FALSE) {
    LOG_ERROR("Error during eglInitialize: {}", FormatEGLError(EglGetError()));
    return;
  }

  if (EglBindAPI(EGL_OPENGL_API) == EGL_FALSE) {
    LOG_ERROR("bind OpenGL API failed");
    return;
  }
  ...
  if (egl_.context == EGL_NO_CONTEXT) {
    LOG_ERROR("Couldn't create EGL context: {}", FormatGLError(EglGetError()));
    return;
  }
  ...
  LOG_INFO("Egl initialization succeeded");
...
bool EglDmaBuf::GetClientExtensions(EGLDisplay dpy, EGLint name) {
  ...
  if (!client_extensions_cstring) {
    ...
    LOG_ERROR("No client extensions defined! {}", FormatEGLError(EglGetError()));
    return false;
  }
...
bool EglDmaBuf::ImageFromDmaBuf(const DesktopSize& size,
  ...
  if (plane_datas.empty()) {
    LOG_ERROR("Failed to process buffer: invalid number of planes");
    return false;
  }
  ...
  if (image == EGL_NO_IMAGE) {
    LOG_ERROR("Failed to record frame: Error creating EGLImage - {}", FormatEGLError(EglGetError()));
    return false;
  }
  ...
  if (GlCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE) {
    LOG_ERROR("Failed to bind DMA buf framebuffer");
  ...
  if (error) {
    LOG_ERROR("Failed to get image from DMA buffer.");
  }
...
std::vector<uint64_t> EglDmaBuf::QueryDmaBufModifiers(uint32_t format) {
  ...
  if (!success || !count) {
    LOG_WARN("Cannot query the number of formats.");
  ...
  if (!EglQueryDmaBufFormatsEXT(egl_.display, count,
                                ...
                                &count)) {
    LOG_WARN("Cannot query a list of formats.");
    return {DRM_FORMAT_MOD_INVALID};
  }

  if (std::find(formats.begin(), formats.end(), drm_format) == formats.end()) {
    LOG_WARN("Format {} not supported for modifiers.", drm_format);
    return {DRM_FORMAT_MOD_INVALID};
  }
  ...
  if (!success || !count) {
    LOG_WARN("Cannot query the number of modifiers.");
    return {DRM_FORMAT_MOD_INVALID};
  }
  ...
  if (!EglQueryDmaBufModifiersEXT(egl_.display, drm_format, count,
    ...
    LOG_WARN("Cannot query a list of modifiers.");
  }
...
std::optional<std::string> EglDmaBuf::GetRenderNode() {
  ...
  if (max_devices <= 0) {
    LOG_ERROR("drmGetDevices2() has not found any devices (errno={})", -max_devices);
    return std::nullopt;
  }
  ...
  if (ret < 0) {
    LOG_ERROR("drmGetDevices2() returned an error {}", ret);
    return std::nullopt;
  }
