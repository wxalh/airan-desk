...
std::unique_ptr<DesktopFrameIOSurface> DesktopFrameIOSurface::Wrap(
  ...
  if (bytes_per_pixel != DesktopFrame::kBytesPerPixel) {
    LOG_ERROR("CGDisplayStream handler returned IOSurface with {} bits per pixel. Only 32-bit depth is supported.",
              (8 * bytes_per_pixel));
  ...
  LOG_TRACE("DesktopFrameIOSurface wrapping IOSurface with size {}x{}. Cropping to ({},{}; {}x{}). Stride={}, buffer-offset-px={}, buffer-offset-bytes={}",
            surface_width, surface_height, offset_columns, offset_rows, width, height,
            stride / bytes_per_pixel, offset / bytes_per_pixel, offset);
