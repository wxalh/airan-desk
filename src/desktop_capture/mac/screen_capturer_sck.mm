...
#include "desktop_capture/mac/desktop_frame_iosurface.h"
...
using airan::desktop_capture::DesktopFrameIOSurface;
...
- (instancetype)initWithCapturer:(airan::desktop_capture::ScreenCapturerSck*)capturer;
...
class API_AVAILABLE(macos(14.0)) ScreenCapturerSck final
 ...
 private:
  ...
  // Airan native descriptor callback. It is invoked on the SCK output thread.
  airan::media::AiranCaptureCallback* airan_capture_callback_
      RTC_GUARDED_BY(lock_) = nullptr;

  // Currently selected display, or 0 if no concrete display has been selected
  // yet. Airan screen capture does not expose full-desktop composition as a
  // screen source.
...
ScreenCapturerSck::~ScreenCapturerSck() {
  ...
  LOG_INFO("ScreenCapturerSck {} destroyed.", fmt::ptr(this));
...
void ScreenCapturerSck::Start(DesktopCapturer::Callback* callback) {
  RTC_DCHECK_RUN_ON(&api_checker_);
  LOG_INFO("ScreenCapturerSck {} {}.", fmt::ptr(this), __func__);
  callback_ = callback;
  desktop_config_ =
...
void ScreenCapturerSck::SetAiranCaptureCallback(
...
void ScreenCapturerSck::SetMaxFrameRate(uint32_t max_frame_rate) {
  RTC_DCHECK_RUN_ON(&api_checker_);
  LOG_DEBUG("ScreenCapturerSck {} SetMaxFrameRate({}).", fmt::ptr(this), max_frame_rate);
...
void ScreenCapturerSck::CaptureFrame() {
  ...
  if (permanent_error_) {
    LOG_TRACE("ScreenCapturerSck {} CaptureFrame() -> ERROR_PERMANENT", fmt::ptr(this));
  ...
  if (frame) {
    LOG_TRACE("ScreenCapturerSck {} CaptureFrame() -> SUCCESS", fmt::ptr(this));
    frame->set_capture_time_ms(webrtc::TimeSince(capture_start_time_millis));
    callback_->OnCaptureResult(Result::SUCCESS, std::move(frame));
  } else {
    LOG_TRACE("ScreenCapturerSck {} CaptureFrame() -> ERROR_TEMPORARY", fmt::ptr(this));
...
void ScreenCapturerSck::EnsurePickerHandle() {
  ...
  if (!picker_handle_ && capture_options_.allow_sck_system_picker()) {
    ...
    LOG_INFO("ScreenCapturerSck {} Created picker handle. allow_sck_system_picker={}, source={}, modes={}",
             fmt::ptr(this), capture_options_.allow_sck_system_picker(),
             (picker_handle_ ? picker_handle_->Source() : -1),
             absl::StrFormat("%v", StringifiableSCContentSharingPickerMode{
...
void ScreenCapturerSck::EnsureVisible() {
  ...
  if (picker_handle_) {
    ...
    LOG_ERROR("ScreenCapturerSck {} EnsureVisible() reached the maximum number of streams.", fmt::ptr(this));
  ...
  if (@available(macOS 15, *)) {
    // Stick with ? because if we use Display, the picker doesn't let us
    ...
    // our current window to show others. Saves a click compared to ? when
    // picking a display.
...
void ScreenCapturerSck::NotifySourceSelection(SCContentFilter* filter,
  ...
  if (stream_ != stream) {
    ...
    LOG_INFO("ScreenCapturerSck {} {}. stream_ != stream.", fmt::ptr(this), __func__);
    return;
  }
  LOG_INFO("ScreenCapturerSck {} {}. Starting.", fmt::ptr(this), __func__);
...
bool ScreenCapturerSck::SelectSource(SourceId id) {
  ...
  LOG_INFO("ScreenCapturerSck {} SelectSource(id={}).", fmt::ptr(this), id);
  bool stream_started = false;
  {
    MutexLock lock(&lock_);
...
void ScreenCapturerSck::OnShareableContentCreated(SCShareableContent* content,
                                                  NSError* error) {
  if (!content) {
    LOG_ERROR("ScreenCapturerSck {} getShareableContent failed with error code {}.",
              fmt::ptr(this), (error ? error.code : 0));
    permanent_error_ = true;
    return;
  }

  if (!content.displays.count) {
    LOG_ERROR("ScreenCapturerSck {} getShareableContent returned no displays.", fmt::ptr(this));
    permanent_error_ = true;
    return;
  }

  MutexLock lock(&lock_);
  LOG_INFO("ScreenCapturerSck {} {}. current_display_={}", fmt::ptr(this), __func__, current_display_);
  ...
  if (!captured_display) {
    if (current_display_ != 0) {
      LOG_ERROR("ScreenCapturerSck {} Display {} not found.", fmt::ptr(this), current_display_);
      permanent_error_ = true;
      return;
    }
    captured_display = content.displays.firstObject;
    current_display_ = captured_display.displayID;
    LOG_INFO("Airan SCK screen capturer maps default/full desktop source to display id {}", current_display_);
  }
...
void ScreenCapturerSck::StartWithFilter(SCContentFilter* __strong filter) {
  ...
  if (stream_) {
    LOG_DEBUG("ScreenCapturerSck {} Updating stream configuration to size={}x{} and max_frame_rate={}.",
              fmt::ptr(this), config.width, config.height, max_frame_rate_);
    [stream_ updateContentFilter:filter completionHandler:nil];
    [stream_ updateConfiguration:config completionHandler:nil];
  } else {
    LOG_INFO("ScreenCapturerSck {} Creating new stream.", fmt::ptr(this));
    ...
    if (!add_stream_output_result) {
      stream_ = nil;
      filter_ = nil;
      LOG_ERROR("ScreenCapturerSck {} addStreamOutput failed.", fmt::ptr(this));
      permanent_error_ = true;
      return;
    }
    ...
    auto handler = ^(NSError* error) {
      if (error) {
        ...
        // this handler.
        permanent_error_ = true;
        LOG_ERROR("ScreenCapturerSck {} Starting failed.", fmt::ptr(this));
      } else {
        LOG_INFO("ScreenCapturerSck {} Capture started.", fmt::ptr(this));
      }
    };
...
void ScreenCapturerSck::OnNewIOSurface(IOSurfaceRef io_surface,
  ...
  airan::media::AiranCaptureCallback* airan_capture_callback = nullptr;
  {
    MutexLock lock(&lock_);
    airan_capture_callback = airan_capture_callback_;
  }
  if (airan_capture_callback) {
    auto retained_surface =
        std::shared_ptr<void>(
            const_cast<void*>(static_cast<const void*>(CFRetain(io_surface))),
            [](void* surface) {
          if (surface) {
            CFRelease(surface);
          }
    ...
    descriptor.native_handle.type = airan::media::NativeHandleType::IOSurface;
    descriptor.native_handle.handle = io_surface;
    descriptor.native_handle.ownership =
        airan::media::NativeHandleOwnership::Retained;
    descriptor.native_handle.retained_owner = retained_surface;
    descriptor.device_id.platform = "macos";
    descriptor.device_id.id = "iosurface";
    descriptor.sync.type = airan::media::SyncType::ImplicitSync;
    descriptor.sync.waiter = airan::media::SyncWaiter::Consumer;
    descriptor.visible_rect = {
        static_cast<int>(img_bounding_rect.origin.x),
        static_cast<int>(img_bounding_rect.origin.y),
        static_cast<int>(img_bounding_rect.size.width),
        static_cast<int>(img_bounding_rect.size.height)};
    descriptor.coded_size = {static_cast<int>(width), static_cast<int>(height)};
    descriptor.planes.push_back(
        {nullptr, 0, 0, descriptor.visible_rect.width,
         descriptor.visible_rect.height, 0});
    descriptor.color_space.primaries = airan::media::ColorValue::Bt709;
    descriptor.color_space.transfer = airan::media::ColorValue::Srgb;
    descriptor.color_space.matrix = airan::media::ColorValue::Bt709;
    descriptor.color_space.range = airan::media::ColorRange::Full;
    descriptor.timestamp = std::chrono::microseconds(webrtc::TimeMicros());
    descriptor.current_capture_path =
        airan::media::CapturePath::NativeGpuCapture;
    descriptor.current_encode_path = airan::media::EncodePath::GpuCopyHwEncode;
    descriptor.fallback_reason = airan::media::FallbackReason::kNone;
    descriptor.release = airan::media::makeFrameRelease([retained_surface]() {});
    airan_capture_callback->onAiranCaptureFrame(std::move(descriptor));
  }

  LOG_TRACE("ScreenCapturerSck {} {}. New surface: width={}, height={}, content_rect={}, bounding_rect={}, overlay_rect=({}, scale_factor={}, content_scale={}. Cropping to rect {}.",
            fmt::ptr(this), __func__, width, height,
...
@implementation SckHelper {
  ...
  airan::desktop_capture::ScreenCapturerSck* _capturer;
}
...
- (void)stream:(SCStream*)stream didStopWithError:(NSError*)error {
  webrtc::MutexLock lock(&_capturer_lock);
  LOG_INFO("ScreenCapturerSck {} {}.", fmt::ptr(_capturer), __func__);
  if (_capturer) {
...
- (void)contentSharingPickerStartDidFailWithError:(NSError*)error {
  webrtc::MutexLock lock(&_capturer_lock);
  LOG_INFO("ScreenCapturerSck {} {}. error.code={}", fmt::ptr(_capturer), __func__, error.code);
  if (_capturer) {
