...
#include "desktop_capture/mac/screen_capturer_mac.h"

#include "common/logger_manager.h"
#include "desktop_capture/mac/desktop_frame_provider.h"
#include "desktop_capture/mac/window_list_utils.h"
...
ScreenCapturerMac::ScreenCapturerMac(
  ...
  LOG_INFO("Allow IOSurface: {}", allow_iosurface);
...
bool ScreenCapturerMac::Init() {
  ...
  if (!desktop_config_.displays.empty()) {
    current_display_ = desktop_config_.displays.front().id;
  }
  return true;
}
...
bool ScreenCapturerMac::SelectSource(SourceId id) {
  if (id == kFullDesktopScreenId) {
    if (desktop_config_.displays.empty()) return false;
    current_display_ = desktop_config_.displays.front().id;
    LOG_INFO("Airan macOS screen capturer maps full desktop source to display id {}", current_display_);
    ScreenConfigurationChanged();
    return true;
  }
...
bool ScreenCapturerMac::CgBlit(const DesktopFrame& frame,
  ...
  const MacDisplayConfiguration* config =
      desktop_config_.FindDisplayConfigurationById(current_display_);
  if (!config && !desktop_config_.displays.empty()) {
    current_display_ = desktop_config_.displays.front().id;
    config = desktop_config_.FindDisplayConfigurationById(current_display_);
  }
  if (config) {
...
void ScreenCapturerMac::ScreenRefresh(CGDirectDisplayID display_id,
  ...
  if (current_display_ && display_id != current_display_) {
    return;
  }

  // The refresh rects are in display coordinates. We want to translate to
  // framebuffer coordinates. Airan screen capture always tracks one concrete
  // display, so no full-desktop translation is needed here.
