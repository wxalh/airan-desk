...
Desktop::~Desktop() {
  if (own_ && desktop_ != NULL) {
    if (!::CloseDesktop(desktop_)) {
      LOG_ERROR("Failed to close the owned desktop handle: {}", GetLastError());
    }
  }
}
...
bool Desktop::GetName(std::wstring* desktop_name_out) const {
  ...
  if (!GetUserObjectInformationW(desktop_, UOI_NAME, &buffer[0],
    ...
    LOG_ERROR("Failed to query the desktop name: {}", GetLastError());
    return false;
  }
...
bool Desktop::SetThreadDesktop() const {
  if (!::SetThreadDesktop(desktop_)) {
    LOG_ERROR("Failed to assign the desktop to the current thread: {}", GetLastError());
    return false;
  }
...
Desktop* Desktop::GetDesktop(const WCHAR* desktop_name) {
  ...
  if (desktop == NULL) {
    LOG_ERROR("Failed to open the desktop '{}': {}", ToUtf8(desktop_name), GetLastError());
    return NULL;
  }
...
Desktop* Desktop::GetThreadDesktop() {
  ...
  if (desktop == NULL) {
    LOG_ERROR("Failed to retrieve the handle of the desktop assigned to the current thread: {}", GetLastError());
    return NULL;
  }
