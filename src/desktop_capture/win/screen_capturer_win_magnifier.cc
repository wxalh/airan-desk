...
#include "desktop_capture/win/screen_capturer_win_magnifier.h"
...
void ScreenCapturerWinMagnifier::CaptureFrame() {
  ...
  if (current_screen_id_ == kInvalidScreenId &&
    ...
    LOG_WARN("No valid screen selected for Magnifier capture.");
    callback_->OnCaptureResult(Result::ERROR_PERMANENT, nullptr);
    return;
  }
  ...
  if (!CaptureImage(rect)) {
    LOG_WARN("Magnifier capturer failed to capture a frame.");
    callback_->OnCaptureResult(Result::ERROR_PERMANENT, nullptr);
    return;
  }
...
bool ScreenCapturerWinMagnifier::SelectSource(SourceId id) {
  if (id == kFullDesktopScreenId) {
    ...
    LOG_INFO("Airan Magnifier screen capturer maps full desktop source to primary screen id {}",id);
  }
...
bool ScreenCapturerWinMagnifier::CaptureImage(const DesktopRect& rect) {
  ...
  if (!result) {
    LOG_WARN("Failed to call SetWindowPos: {}. Rect = {{{}, {}, {}, {}}}",
  ...
  if (!result) {
    LOG_WARN("Failed to call MagSetWindowSource: {}. Rect = {{{}, {}, {}, {}}}",
             GetLastError(), rect.left(), rect.top(), rect.right(), rect.bottom());
    return false;
  }
...
bool ScreenCapturerWinMagnifier::InitializeMagnifier() {
  ...
  if (GetSystemMetrics(SM_CMONITORS) != 1) {
    ...
    // crashes sometimes).
    LOG_WARN("Magnifier capturer cannot work on multi-screen system.");
    return false;
  }
  ...
  if (!mag_initialize_func_ || !mag_uninitialize_func_ ||
      ...
      !set_image_scaling_callback_func_) {
    LOG_WARN("Failed to initialize ScreenCapturerWinMagnifier: "
  ...
  if (!magnifier_window_) {
    mag_uninitialize_func_();
    LOG_WARN("Failed to initialize ScreenCapturerWinMagnifier: "
             "error from creating magnifier window {}", GetLastError());
    return false;
  }
  ...
  if (!result) {
    mag_uninitialize_func_();
    LOG_WARN("Failed to initialize ScreenCapturerWinMagnifier: "
             "error from MagSetImageScalingCallback {}", GetLastError());
    return false;
  }
...
void ScreenCapturerWinMagnifier::OnCaptured(void* data,
  ...
  int captured_bytes_per_pixel = ClampToInt(
      static_cast<int64_t>(header.cbSize) / header.width / header.height);
  if (header.format != GUID_WICPixelFormat32bppRGBA ||
    ...
    LOG_WARN("Output format does not match the captured format: "
             "width = {}, height = {}, stride = {}, bpp = {}, pixel format RGBA ? {}.",
