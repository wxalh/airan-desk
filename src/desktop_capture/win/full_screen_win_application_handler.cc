...
#include "desktop_capture/win/full_screen_win_application_handler.h"
...
std::string WindowText(HWND window) {
  ...
  size_t copied = ::GetWindowTextW(window, buffer.data(),
                                  static_cast<int>(buffer.size()));
  if (copied == 0) {
    return std::string();
  }
  if (copied > static_cast<size_t>((std::numeric_limits<int>::max)())) {
    copied = static_cast<size_t>((std::numeric_limits<int>::max)());
  }
  return ToUtf8(buffer.data(), static_cast<int>(copied));
}
...
std::string FullScreenPowerPointHandler::GetDocumentTitleFromSlideShow(
  ...
  if (position != std::string::npos) {
    const absl::string_view trimmed = absl::StripAsciiWhitespace(
        absl::string_view(title).substr(position + 1, std::string::npos));
    title.assign(trimmed.data(), trimmed.size());
  }
