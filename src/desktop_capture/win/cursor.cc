...
#include "desktop_capture/win/scoped_gdi_object.h"
...
MouseCursor* CreateMouseCursorFromHCursor(HDC dc, HCURSOR cursor) {
  ...
  if (!GetIconInfo(cursor, &iinfo)) {
    LOG_ERROR("Unable to get cursor icon info. Error = {}", GetLastError());
    return NULL;
  }
  ...
  if (!GetObject(scoped_mask, sizeof(bitmap_info), &bitmap_info)) {
    LOG_ERROR("Unable to get bitmap info. Error = {}", GetLastError());
    return NULL;
  }
  ...
  if (!GetDIBits(dc, scoped_mask, 0, height, mask_data.get(),
    ...
    LOG_ERROR("Unable to get bitmap bits. Error = {}", GetLastError());
    return NULL;
  }
