...
#include "desktop_capture/win/wgc_capture_session.h"
...
void RecordGetFirstFrameTime(int64_t elapsed_time_ms) {
  const int elapsed_time_ms_clamped =
      static_cast<int>(std::clamp<int64_t>(elapsed_time_ms, 1, 5000));
  RTC_HISTOGRAM_COUNTS(
      "WebRTC.DesktopCapture.Win.WgcCaptureSessionTimeToFirstFrame",
      elapsed_time_ms_clamped, /*min=*/1, /*max=*/5000, /*bucket_count=*/100);
}
...
void WgcCaptureSession::SetAiranCaptureCallback(
    airan::media::AiranCaptureCallback* callback) {
  RTC_DCHECK_RUN_ON(&sequence_checker_);
  airan_capture_callback_ = callback;
  if (!airan_capture_callback_) {
    airan_has_native_frame_ = false;
    airan_native_frame_emitted_ = false;
  }
}
...
bool WgcCaptureSession::WaitForFirstFrame() {
  ...
  if (!has_first_frame_arrived_event_->Wait(
    ...
    LOG_ERROR("Timed out after waiting {} ms for the first frame.", kFirstFrameTimeoutMs);
    return false;
  }
...
void WgcCaptureSession::EnsureFrame() {
  ...
  if (SUCCEEDED(hr)) {
    if (airan_native_frame_emitted_ || airan_has_native_frame_) {
      return;
    }
  ...
  if (queue_.current_frame()) {
    LOG_TRACE("ProcessFrame failed, using existing frame: {}", hr);
    return;
  }
  ...
  int sleep_count = 0;
  while (!queue_.current_frame() && !airan_native_frame_emitted_ &&
         sleep_count < max_sleep_count) {
    sleep_count++;
    Thread::SleepMs(sleep_time_ms);
    hr = ProcessFrame();
    if (FAILED(hr)) {
      LOG_TRACE("ProcessFrame failed during startup: {}", hr);
    }
  }
  if (!queue_.current_frame() && !airan_native_frame_emitted_)
    LOG_WARN("Unable to process a valid frame even after trying 10 times.");
}
...
bool WgcCaptureSession::EmitAiranNativeFrame(
  ...
  RTC_DCHECK_RUN_ON(&sequence_checker_);
  airan_native_frame_emitted_ = false;
  if (!airan_capture_callback_ || !texture || width <= 0 || height <= 0) {
    return false;
  }
  ...
  const int visible_width =
      std::min(width, static_cast<int>(texture_desc.Width));
  const int visible_height =
      std::min(height, static_cast<int>(texture_desc.Height));
  if (visible_width <= 0 || visible_height <= 0) {
    return false;
  }
  if (visible_width != width || visible_height != height) {
    LOG_WARN("Airan WGC native frame content size exceeds texture size during resize: content={}x{}, texture={}x{}; using visible={}x{}",
             width, height, texture_desc.Width, texture_desc.Height, visible_width, visible_height);
  }
  D3D11_TEXTURE2D_DESC retained_desc = texture_desc;
  retained_desc.Usage = D3D11_USAGE_DEFAULT;
  retained_desc.BindFlags = D3D11_BIND_SHADER_RESOURCE |
                            D3D11_BIND_RENDER_TARGET;
  retained_desc.CPUAccessFlags = 0;
  retained_desc.MiscFlags = 0;

  ComPtr<ID3D11Texture2D> retained_texture;
  HRESULT hr =
      d3d11_device_->CreateTexture2D(&retained_desc, nullptr, &retained_texture);
  if (FAILED(hr) || !retained_texture) {
    LOG_WARN("Airan WGC native frame skipped: failed to create retained D3D11 texture, hr={}", hr);
    return false;
  }
  ...
  if (!d3d_context) {
    LOG_WARN("Airan WGC native frame skipped: missing D3D11 immediate context");
    return false;
  }
  d3d_context->CopyResource(retained_texture.Get(), texture.Get());

  uint64_t adapter_luid = 0;
  ComPtr<IDXGIDevice> dxgi_device;
  if (d3d11_device_ &&
      SUCCEEDED(d3d11_device_->QueryInterface(IID_PPV_ARGS(&dxgi_device)))) {
    ComPtr<IDXGIAdapter> adapter;
    if (SUCCEEDED(dxgi_device->GetAdapter(&adapter)) && adapter) {
      DXGI_ADAPTER_DESC adapter_desc = {};
      if (SUCCEEDED(adapter->GetDesc(&adapter_desc))) {
        adapter_luid =
            (static_cast<uint64_t>(adapter_desc.AdapterLuid.HighPart) << 32) |
            static_cast<uint32_t>(adapter_desc.AdapterLuid.LowPart);
      }
    }
  }

  auto retained_texture_owner =
      std::make_shared<ComPtr<ID3D11Texture2D>>(std::move(retained_texture));
  airan::media::CaptureFrameDescriptor descriptor;
  descriptor.frame_kind = airan::media::FrameKind::NativeGpu;
  descriptor.capture_backend = "wgc";
  descriptor.native_handle.type =
      airan::media::NativeHandleType::D3D11Texture2D;
  descriptor.native_handle.handle = retained_texture_owner->Get();
  descriptor.native_handle.ownership =
      airan::media::NativeHandleOwnership::Retained;
  descriptor.native_handle.retained_owner = retained_texture_owner;
  descriptor.native_handle.subresource = 0;
  descriptor.device_id.platform = "windows";
  descriptor.device_id.id = "d3d11";
  descriptor.device_id.luid = adapter_luid;
  descriptor.sync.type = airan::media::SyncType::ImplicitSync;
  descriptor.sync.waiter = airan::media::SyncWaiter::Consumer;
  descriptor.visible_rect = {0, 0, visible_width, visible_height};
  descriptor.coded_size = {static_cast<int>(texture_desc.Width),
                           static_cast<int>(texture_desc.Height)};
  descriptor.planes.push_back({nullptr, 0, 0, visible_width, visible_height, 0});
  ...
  descriptor.timestamp = std::chrono::microseconds(timestamp_us);
  descriptor.current_capture_path =
      airan::media::CapturePath::NativeGpuCapture;
  descriptor.current_encode_path =
      airan::media::EncodePath::GpuCopyHwEncode;
  descriptor.fallback_reason = airan::media::FallbackReason::kNone;
  descriptor.release = airan::media::makeFrameRelease([retained_texture_owner]() {
    retained_texture_owner->Reset();
  });

  airan_capture_callback_->onAiranCaptureFrame(std::move(descriptor));
  airan_native_frame_emitted_ = true;
  airan_has_native_frame_ = true;
  return true;
}

HRESULT WgcCaptureSession::RecreateFramePoolForSize(
    ComPtr<ID3D11Texture2D> texture,
    ABI::Windows::Graphics::SizeInt32 size) {
  RTC_DCHECK_RUN_ON(&sequence_checker_);
  if (!SizeHasChanged(size, size_)) {
    return S_OK;
  }
  ...
  queue_.Reset();
  damage_region_.Clear();
  HRESULT hr = frame_pool_->Recreate(direct3d_device_.Get(), kPixelFormat,
                                     kNumBuffers, size);
  if (FAILED(hr)) {
  ...
  // Keep the CPU fallback texture in sync with the new WGC frame-pool size.
  hr = CreateMappedTexture(texture, size.Width, size.Height);
  if (FAILED(hr)) {
  ...
  size_ = size;
  LOG_DEBUG("Airan WGC frame pool resized for native capture: {}x{}", size.Width, size.Height);
  return S_OK;
}
...
HRESULT WgcCaptureSession::ProcessFrame() {
  ...
  if (!capture_frame) {
    if (!queue_.current_frame()) {
      if (airan_has_native_frame_) {
        LOG_TRACE("Frame pool was empty; keeping last native WGC frame.");
        RecordGetFrameResult(GetFrameResult::kFramePoolEmpty);
        return S_FALSE;
      }
      // The frame pool was empty and so is the external queue.
      LOG_TRACE("Frame pool was empty => kFramePoolEmpty.");
      RecordGetFrameResult(GetFrameResult::kFramePoolEmpty);
    } else {
  ...
  if (queue_.current_frame() && queue_.current_frame()->IsShared()) {
    LOG_TRACE("Overwriting frame that is still shared.");
  }
  ...
  // Airan modification: emit the native D3D11 frame before WebRTC's staging
  // readback path. The CPU DesktopFrame path below remains as fallback when no
  // native callback is active or when native descriptor emission fails.
  hr = RecreateFramePoolForSize(texture_2D, new_size);
  if (FAILED(hr)) {
    return hr;
  }
  if (EmitAiranNativeFrame(texture_2D, new_size.Width, new_size.Height,
                           TimeMicros())) {
  ...
  int image_width = std::min({size_.Width, new_size.Width,
                              static_cast<int>(texture_desc.Width)});
  ...
  if (is_window_source_) {
    ...
    if (!monitor_.has_value()) {
      ...
      if (!GetHmonitorFromDeviceIndex(source_id_, &monitor)) {
        LOG_ERROR("Failed to get HMONITOR from device index.");
  ...
  if (FAILED(scale_factor_hr))
    LOG_ERROR("Failed to get scale factor for monitor: {}", scale_factor_hr);
...
void WgcCaptureSession::RemoveFrameArrivedEventHandler() {
  ...
  if (frame_arrived_token_) {
    ...
    if (FAILED(hr)) {
      LOG_WARN("Failed to remove FrameArrived event handler: {}", hr);
    }
  }
}
...
HRESULT WgcCaptureSession::AddFrameArrivedEventHandler() {
  ...
  if (FAILED(hr)) {
    LOG_WARN("Failed to add FrameArrived event handler: {}", hr);
    frame_arrived_token_.reset();
    has_first_frame_arrived_event_ = nullptr;
  }
  return hr;
