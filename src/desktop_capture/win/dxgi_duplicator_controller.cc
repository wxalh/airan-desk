...
#include "desktop_capture/win/dxgi_adapter_duplicator.h"
...
DWORD GetCurrentSessionId() {
  ...
  if (!::ProcessIdToSessionId(::GetCurrentProcessId(), &session_id)) {
    LOG_WARN("Failed to retrieve current session Id, current binary may not have required priviledge.");
  }
  return session_id;
}
...
void DxgiDuplicatorController::Release() {
  ...
  RTC_DCHECK(refcount >= 0);
  if (refcount == 0) {
    LOG_WARN("Count of references reaches zero, DxgiDuplicatorController will be unloaded.");
    Unload();
  }
}
...
bool DxgiDuplicatorController::RetrieveD3dInfo(D3dInfo* info) {
  ...
  if (!result) {
    LOG_WARN("Failed to initialize DXGI components, the D3dInfo retrieved may not accurate or out of date.");
  }
  return result;
}
...
DxgiDuplicatorController::Result DxgiDuplicatorController::Duplicate(
  ...
  return DoDuplicate(frame, -1, callback);
}
...
DxgiDuplicatorController::Result DxgiDuplicatorController::DuplicateMonitor(
  ...
  return DoDuplicate(frame, monitor_id, callback);
}
...
bool DxgiDuplicatorController::DoDuplicateUnlocked(Context* context,
  ...
  if (monitor_id < 0) {
    ...
    result = DoDuplicateOne(context, monitor_id, target, callback);
  }
...
bool DxgiDuplicatorController::EnsureFrameCaptured(Context* context,
  ...
  while (GetNumFramesCaptured(monitor_id) < frames_to_skip) {
    if (monitor_id < 0) {
      ...
      if (!DoDuplicateOne(context, monitor_id, shared_frame, nullptr)) {
        return false;
      }
    }
    ...
    if (TimeMillis() - start_ms > timeout_ms) {
      LOG_ERROR("Failed to capture {} frames within {} milliseconds.",
                frames_to_skip, timeout_ms);
      return false;
    }
