...
#include "desktop_capture/desktop_frame_win.h"
...
bool HasInvalidSourceRect(const DesktopCapturer::SourceList& sources) {
  if (sources.empty())
    return true;
  for (const auto& source : sources) {
    if (GetScreenRect(source.id, std::nullopt).is_empty())
      return true;
  }
  return false;
}
...
bool ScreenCapturerWinGdi::GetSourceList(SourceList* sources) {
  SourceList device_sources;
  const bool device_result = GetScreenList(&device_sources);
  SourceList monitor_sources;
  const bool monitor_result = GetAiranMonitorList(&monitor_sources);
  const bool device_rect_invalid = HasInvalidSourceRect(device_sources);
  if (monitor_result &&
      (monitor_sources.size() > device_sources.size() ||
       !device_result || device_sources.empty() || device_rect_invalid)) {
    LOG_INFO("Airan GDI screen capturer uses monitor source list: {} monitors, {} display devices, deviceRectInvalid={}",
             monitor_sources.size(), device_sources.size(), device_rect_invalid);
    *sources = std::move(monitor_sources);
    return true;
  }
  *sources = std::move(device_sources);
...
bool ScreenCapturerWinGdi::SelectSource(SourceId id) {
  if (id == kFullDesktopScreenId) {
    ...
    LOG_INFO("Airan GDI screen capturer maps full desktop source to primary screen id {}", id);
  }
...
bool ScreenCapturerWinGdi::CaptureImage() {
  if (current_screen_id_ == kInvalidScreenId) {
    if (!SelectSource(kFullDesktopScreenId)) {
      LOG_WARN("No valid per-screen source selected for GDI capture; falling back to full virtual desktop.");
    }
  }

  DesktopRect screen_rect = current_screen_id_ == kInvalidScreenId
                                ? GetFullscreenRect()
                                : GetScreenRect(current_screen_id_,
                                                current_device_key_);
  if (screen_rect.is_empty()) {
    screen_rect = GetFullscreenRect();
    if (screen_rect.is_empty()) {
      LOG_WARN("Failed to get screen rect.");
      return false;
    }
    LOG_WARN("Selected GDI screen rect is empty; falling back to full virtual desktop.");
  }
  ...
  if (!result) {
    LOG_WARN("BitBlt failed: {}", GetLastError());
  }
