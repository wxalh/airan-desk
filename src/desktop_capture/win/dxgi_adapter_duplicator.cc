...
bool DxgiAdapterDuplicator::DoInitialize() {
  for (int i = 0;; i++) {
    ...
    if (error.Error() == DXGI_ERROR_NOT_CURRENTLY_AVAILABLE) {
      LOG_WARN("IDXGIAdapter::EnumOutputs returned NOT_CURRENTLY_AVAILABLE. This may happen when running in session 0.");
      break;
    }
    ...
    if (error.Error() == S_OK) {
      if (desc.AttachedToDesktop && IsValidRect(desc.DesktopCoordinates)) {
        ...
        if (error.Error() != S_OK || !output1) {
          LOG_WARN("Failed to convert IDXGIOutput to IDXGIOutput1, this usually means the system does not support DirectX 11");
          continue;
        }
      ...
      LOG_WARN("Failed to get output description of device {}, ignore.", i);
    }
  }
