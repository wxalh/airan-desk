...
#include "desktop_capture/win/window_capture_utils.h"
...
HRESULT WgcWindowSource::CreateCaptureItem(
  ...
  if (!ResolveCoreWinRTDelayload()) {
    LOG_ERROR("Failed to load CoreWinRT library");
    return E_FAIL;
  }
  ...
  if (FAILED(hr)) {
    LOG_ERROR("GetActivationFactory failed with hr: {}", hr);
    return hr;
  }
  ...
  hr = interop->CreateForWindow(reinterpret_cast<HWND>(GetSourceId()),
                                IID_PPV_ARGS(&item));
  if (FAILED(hr)) {
    LOG_ERROR("CreateForWindow failed with hr: {}", hr);
    return hr;
  }

  if (!item) {
    LOG_ERROR("CreateForWindow returned null IGraphicsCaptureItem");
    return E_HANDLE;
  }
...
WgcScreenSource::WgcScreenSource(DesktopCapturer::SourceId source_id)
    : WgcCaptureSource(source_id == kFullDesktopScreenId ? GetPrimaryScreenId()
                                                         : source_id) {
  if (source_id == kFullDesktopScreenId) {
    LOG_INFO("Airan WGC screen source maps full desktop source to primary screen id {}", GetSourceId());
  }
...
HRESULT WgcScreenSource::CreateCaptureItem(
  ...
  if (FAILED(hr)) {
    LOG_ERROR("CreateForMonitor failed with hr: {}", hr);
    return hr;
  }

  if (!item) {
    LOG_ERROR("CreateForMonitor returned null IGraphicsCaptureItem");
    return E_HANDLE;
  }
