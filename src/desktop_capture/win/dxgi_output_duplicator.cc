...
#include "desktop_capture/win/dxgi_output_duplicator.h"
...
#include "desktop_capture/desktop_frame_rotation.h"
...
#include "desktop_capture/win/dxgi_texture_staging.h"
...
bool DxgiOutputDuplicator::DuplicateOutput() {
  ...
  if (error.Error() != S_OK || !duplication_) {
    LOG_WARN("Failed to duplicate output from IDXGIOutput1: {}",
  ...
  if ((desc_.ModeDesc.Format != DXGI_FORMAT_B8G8R8A8_UNORM) &&
    ...
    LOG_ERROR("IDXGIDuplicateOutput does not use RGBA (8, 16 bit) which is required by downstream components format is {}",
  ...
  if (static_cast<int>(desc_.ModeDesc.Width) != desktop_rect_.width() ||
    ...
    LOG_ERROR("IDXGIDuplicateOutput does not return a same size as its IDXGIOutput1, size returned by IDXGIDuplicateOutput is {} x {}, size returned by IDXGIOutput1 is {} x {}",
              desc_.ModeDesc.Width, desc_.ModeDesc.Height, desktop_rect_.width(), desktop_rect_.height());
    return false;
  }
...
bool DxgiOutputDuplicator::ContainsMouseCursor(
  ...
  // AcquireNextFrame indicates that a separate pointer is not visible, hence
...
bool DxgiOutputDuplicator::EmitAiranNativeFrame(
    const ComPtr<IDXGIResource>& resource,
    const DXGI_OUTDUPL_FRAME_INFO& /* frame_info */,
  ...
  _com_error error = resource->QueryInterface(__uuidof(ID3D11Texture2D),
                                              &source_texture);
  if (error.Error() != S_OK || !source_texture) {
    LOG_WARN("Airan DXGI native frame skipped: IDXGIResource is not ID3D11Texture2D, error={}",
             desktop_capture::utils::ComErrorToString(error));
    return false;
  }
  ...
  if (!supported_format) {
    LOG_DEBUG("Airan DXGI native frame skipped: unsupported texture format={}", static_cast<int>(source_desc.Format));
    return false;
  }

  D3D11_TEXTURE2D_DESC retained_desc = {};
  retained_desc.Width = source_desc.Width;
  retained_desc.Height = source_desc.Height;
  retained_desc.MipLevels = 1;
  retained_desc.ArraySize = 1;
  retained_desc.Format = source_desc.Format;
  retained_desc.SampleDesc.Count = 1;
  retained_desc.SampleDesc.Quality = 0;
  retained_desc.Usage = D3D11_USAGE_DEFAULT;
  retained_desc.BindFlags =
  ...
  error = device_.d3d_device()->CreateTexture2D(
      &retained_desc, nullptr, retained_texture.GetAddressOf());
  if (error.Error() != S_OK || !retained_texture) {
    LOG_WARN("Airan DXGI native frame skipped: failed to create retained D3D11 texture, error={}",
             desktop_capture::utils::ComErrorToString(error));
    return false;
  }
  ...
  const D3D11_BOX source_box = {0,
                                ...
                                source_desc.Height,
                                1};
  device_.context()->CopySubresourceRegion(retained_texture.Get(), 0, 0, 0, 0,
                                           source_texture.Get(), 0,
                                           &source_box);

  uint64_t adapter_luid = 0;
  if (device_.dxgi_adapter()) {
    DXGI_ADAPTER_DESC adapter_desc = {};
    if (SUCCEEDED(device_.dxgi_adapter()->GetDesc(&adapter_desc))) {
      adapter_luid =
  ...
  descriptor.visible_rect = {0, 0, desktop_size().width(),
                             desktop_size().height()};
  descriptor.coded_size = {static_cast<int>(source_desc.Width),
                           static_cast<int>(source_desc.Height)};
  descriptor.planes.push_back({nullptr, 0, 0, descriptor.visible_rect.width,
  ...
  descriptor.timestamp = std::chrono::microseconds(TimeMicros());
  ...
  descriptor.release = airan::media::makeFrameRelease([retained_owner]() {
...
bool DxgiOutputDuplicator::Duplicate(Context* context,
                                     ...
                                     airan::media::AiranCaptureCallback*
                                         callback) {
  RTC_DCHECK(duplication_);
  RTC_DCHECK(texture_);
  RTC_DCHECK(target);
  ...
  if (error.Error() != S_OK && error.Error() != DXGI_ERROR_WAIT_TIMEOUT) {
    LOG_ERROR("Failed to capture frame: {}", desktop_capture::utils::ComErrorToString(error));
    return false;
  }
  ...
  if (error.Error() == S_OK && frame_info.AccumulatedFrames > 0 && resource) {
    if (offset.is_zero()) {
      EmitAiranNativeFrame(resource, frame_info, callback);
    }
...
bool DxgiOutputDuplicator::DoDetectUpdatedRegion(
  ...
  if (error.Error() != S_OK) {
    LOG_ERROR("Failed to get move rectangles: {}", desktop_capture::utils::ComErrorToString(error));
    return false;
  }
  ...
  if (error.Error() != S_OK) {
    LOG_ERROR("Failed to get dirty rectangles: {}", desktop_capture::utils::ComErrorToString(error));
    return false;
  }
  ...
  while (move_rects_count > 0) {
    ...
    if (move_rects->SourcePoint.x != move_rects->DestinationRect.left ||
      ...
      LOG_INFO("Unmoved move_rect detected, [{}, {}] - [{}, {}].",
               move_rects->DestinationRect.left,
               move_rects->DestinationRect.top,
               move_rects->DestinationRect.right,
               move_rects->DestinationRect.bottom);
...
std::optional<float> DxgiOutputDuplicator::device_scale_factor() const {
  ...
  if (FAILED(hr)) {
    LOG_ERROR("Failed to get scale factor for monitor: {}", hr);
    return std::nullopt;
  }
