...
#include "desktop_capture/win/wgc_capture_source.h"
...
UINT32 HstringLength(const wchar_t* value) {
  const size_t length = wcslen(value);
  RTC_DCHECK_LE(length, static_cast<size_t>((std::numeric_limits<UINT32>::max)()));
...
void LogDirtyRegionSupport() {
  ...
  hr = CreateHstring(kDirtyRegionMode, HstringLength(kDirtyRegionMode),
                     &dirty_region_mode);
  if (FAILED(hr)) {
  ...
  hr = CreateHstring(kWgcSessionType, HstringLength(kWgcSessionType),
                     &wgc_session_type);
  if (SUCCEEDED(hr)) {
    boolean is_dirty_region_mode_supported = false;
    hr = api_info_statics->IsPropertyPresent(
...
bool IsWgcSupported(CaptureType capture_type) {
  if (!HasActiveDisplay()) {
    ...
    if (capture_type == CaptureType::kScreen) {
      LOG_WARN("WGC screen capture unsupported: no active display");
      return false;
    }
    ...
    if (rtc_win::GetVersion() < rtc_win::Version::VERSION_WIN11) {
      LOG_WARN("WGC capture unsupported: no active display and Windows version {} is older than Win11",
               static_cast<int>(rtc_win::GetVersion()));
      return false;
    }
  }
  ...
  if (capture_type == CaptureType::kScreen &&
    ...
    LOG_WARN("WGC screen capture unsupported: Windows version {} is older than Win10 20H1",
             static_cast<int>(rtc_win::GetVersion()));
    return false;
  }

  if (!ResolveCoreWinRTDelayload()) {
    LOG_WARN("WGC capture unsupported: Core WinRT delay-load failed");
    return false;
  }
  ...
  HRESULT hr = GetActivationFactory<
      ...
      &api_info_statics);
  if (FAILED(hr)) {
    LOG_WARN("WGC capture unsupported: ApiInformation activation failed, hr={}", hr);
    return false;
  }

  HSTRING api_contract;
  hr = CreateHstring(kApiContract, HstringLength(kApiContract), &api_contract);
  if (FAILED(hr)) {
    LOG_WARN("WGC capture unsupported: ApiContract hstring failed, hr={}", hr);
    return false;
  }
  ...
  if (FAILED(hr) || !is_api_present) {
    LOG_WARN("WGC capture unsupported: UniversalApiContract major {} not present, hr={}, present={}",
             kRequiredApiContractVersion, hr, static_cast<int>(is_api_present));
    return false;
  }
  ...
  hr = CreateHstring(kWgcSessionType, HstringLength(kWgcSessionType),
                     &wgc_session_type);
  if (FAILED(hr)) {
    LOG_WARN("WGC capture unsupported: session hstring failed, hr={}", hr);
    return false;
  }
  ...
  if (FAILED(hr) || !is_type_present) {
    LOG_WARN("WGC capture unsupported: GraphicsCaptureSession type missing, hr={}, present={}",
             hr, static_cast<int>(is_type_present));
    return false;
  }
  ...
  if (FAILED(hr)) {
    LOG_WARN("WGC capture unsupported: GraphicsCaptureSession activation failed, hr={}", hr);
    return false;
  }
  ...
  if (FAILED(hr) || !is_supported) {
    LOG_WARN("WGC capture unsupported: GraphicsCaptureSession::IsSupported failed, hr={}, supported={}",
             hr, static_cast<int>(is_supported));
    return false;
  }

  LOG_INFO("WGC capture supported for type={}", (capture_type == CaptureType::kScreen ? "screen" : "window"));
  return true;
}
...
void WgcCapturerWin::SetAiranCaptureCallback(
    airan::media::AiranCaptureCallback* callback) {
  airan_capture_callback_ = callback;
  for (auto& capture : ongoing_captures_) {
    capture.second.SetAiranCaptureCallback(callback);
  }
}
...
void WgcCapturerWin::CaptureFrame() {
  ...
  if (session_iter == ongoing_captures_.end()) {
    ...
    capture_session->SetAiranCaptureCallback(airan_capture_callback_);
  } else {
  ...
  if (!capture_session->GetFrame(&frame,
    ...
    LOG_WARN("GetFrame failed; treating WGC frame acquisition as temporary so it can be retried.");
    ongoing_captures_.erase(capture_source_->GetSourceId());
  ...
  int capture_time_ms = ClampToInt(
