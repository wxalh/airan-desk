...
#include "desktop_capture/full_screen_window_detector.h"
#include "desktop_capture/win/screen_capture_utils.h"
#include "desktop_capture/win/selected_window_context.h"
  ...
  bool IsTopWindow() {
    ...
    while (true) {
      ...
      if (lastError == ERROR_SUCCESS) {
        ...
        if (++num_retries <= 10) {
          LOG_WARN("Enumeration failed due to race with a window closing; retrying - retry #{}", num_retries);
          continue;
        } else {
          LOG_ERROR("Exhausted retry allowance around window enumeration failures due to races with windows closing");
        }
      }
      ...
      LOG_ERROR("Failed to enumerate windows: {}", lastError);
    ...
    if (full_screen_window_detector_) {
      full_screen_window_detector_->UpdateWindowListIfNeeded(
          selected_window(), [this](DesktopCapturer::SourceList* sources) {
            ...
            result.erase(
                std::remove_if(result.begin(), result.end(), [this](const auto& source) {
...
bool CroppingWindowCapturerWin::ShouldUseScreenCapturer() {
  ...
  if (region_type == SIMPLEREGION) {
    ...
    // the point of the intersecting operationsNone Why cannot we directly retrieve
    // `window_region_rect_` from GetWindowRegionTypeWithBoundary() functionNone
